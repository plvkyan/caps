...
const AdminReservationForm = ({ }) => {
    ...
    useEffect(() => {
        ...
        const fetchUnrejectedReservations = async () => {

            const response = await fetch('http://localhost:4000/api/reservations/unrejected')
        ...
        fetchUnrejectedReservations()



        const fetchAmenity = async () => {

            const response = await fetch( 'https://localhost:4000/api/amenities/');

            if ( response.ok ) {
                
                setAmenity( await response.json() )
    ...
    const handleChange = (e) => {
        ...
        const filteredAmenity = amenity.filter( a => a.amenityName === amenityName);

        if ( (reservationQuantity as any) > filteredAmenity[0].amenityQuantityMax ) {

            setReservationCount( filteredAmenity[0].amenityQuantityMax )

        }

        if ( (reservationQuantity as any) < filteredAmenity[0].amenityQuantityMin ) {

            setReservationCount( filteredAmenity[0].amenityQuantityMin )

        }

        setAmenityCount( filteredAmenity[0].amenityQuantity - ( reservationQuantity as any ) )
                                                        ...
                                                        <Card x-chunk="dashboard-07-chunk-0">
                                                            ...
                                                            <CardContent>
                                                                ...
                                                                <div className="grid">
                                                                    ...
                                                                    {/* Seventh FormField */}
                                                                    {(amenityType === "Equipment" && amenityName === "Chairs" && amenity) && (
                                                                            ...
                                                                            {form.getValues("amenityName") != "" &&
                                                                                (
                                                                                    <FormDescription className="text-sm text-end">
                                                                                        {form.getValues("amenityName")} left: { amenityCount }
                                                                                    </FormDescription>
                                                                                )
                                                                            }
                                                                    ...
                                                                    {(amenityType === "Equipment" && amenityName === "Tables" && amenity) && (
                                                                    ...
                                                                    {(amenityType === "Equipment" && amenityName === "Tents" && amenity) && (
